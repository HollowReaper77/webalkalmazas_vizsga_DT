...
--
-- Gép: 127.0.0.1
-- Létrehozás ideje: 2024. Máj 24. 13:52
...
(1, 'Teszt Felhasználó', 'teszt@turr.hu', 'Ab123456');

-- --------------------------------------------------------

--
-- A nézet helyettes szerkezete `grafikon`
-- (Lásd alább az aktuális nézetet)
--
CREATE TABLE `grafikon` (
`honap` int(2)
,`osszeg` decimal(32,0)
);
...
--
-- A nézet helyettes szerkezete `javitasok_`
-- (Lásd alább az aktuális nézetet)
--
CREATE TABLE `javitasok_` (
`ID` int(3)
,`Szerelő` varchar(30)
...
`felhasznalok` bigint(21)
,`tulajdonosok` bigint(21)
,`javitasok` bigint(21)
,`szerelok` bigint(21)
,`tipusok` bigint(21)
...
--
-- A nézet helyettes szerkezete `szerelok_`
-- (Lásd alább az aktuális nézetet)
--
CREATE TABLE `szerelok_` (
`ID` int(2)
,`Név` varchar(30)
,`Lakcím` varchar(34)
...
--
-- A nézet helyettes szerkezete `tipusok_`
-- (Lásd alább az aktuális nézetet)
--
CREATE TABLE `tipusok_` (
`ID` int(2)
,`Kerékpártípus` varchar(20)
,`Méret` int(2)
);
...
--
-- A nézet helyettes szerkezete `tulajdonos_`
-- (Lásd alább az aktuális nézetet)
--
CREATE TABLE `tulajdonos_` (
`ID` int(2)
,`Név` varchar(30)
,`Kerékpártípus` varchar(20)
,`Telefonszám` varchar(14)
...
--
-- Nézet szerkezete `grafikon`
--
DROP TABLE IF EXISTS `grafikon`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `grafikon`  AS SELECT month(`javitasok`.`datum`) AS `honap`, sum(`javitasok`.`iranyar`) AS `osszeg` FROM `javitasok` GROUP BY month(`javitasok`.`datum`) ;

-- --------------------------------------------------------

--
-- Nézet szerkezete `javitasok_`
--
DROP TABLE IF EXISTS `javitasok_`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `javitasok_`  AS SELECT `javitasok`.`javitas_id` AS `ID`, `szerelok`.`nev` AS `Szerelő`, `tulajdonos`.`nev` AS `Tulajdonos`, `javitasok`.`datum` AS `Dátum`, `javitasok`.`javido` AS `Javításidő`, `javitasok`.`iranyar` AS `Irányár` FROM ((`javitasok` join `szerelok` on(`szerelok`.`szerelo_id` = `javitasok`.`szerelo_id`)) join `tulajdonos` on(`tulajdonos`.`tulaj_id` = `javitasok`.`tulaj_id`)) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `statisztika`  AS SELECT (select count(0) from `felhasznalok`) AS `felhasznalok`, (select count(0) from `tulajdonos`) AS `tulajdonosok`, (select count(0) from `javitasok`) AS `javitasok`, (select count(0) from `szerelok`) AS `szerelok`, (select count(0) from `tipusok`) AS `tipusok`, (select sum(`javitasok`.`iranyar`) from `javitasok`) AS `bevetel` ;

-- --------------------------------------------------------

--
-- Nézet szerkezete `szerelok_`
--
DROP TABLE IF EXISTS `szerelok_`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `szerelok_`  AS SELECT `szerelok`.`szerelo_id` AS `ID`, `szerelok`.`nev` AS `Név`, `szerelok`.`cim` AS `Lakcím`, `szerelok`.`telefon` AS `Telefonszám` FROM `szerelok` ;

-- --------------------------------------------------------

--
-- Nézet szerkezete `tipusok_`
--
DROP TABLE IF EXISTS `tipusok_`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `tipusok_`  AS SELECT `tipusok`.`tipus_id` AS `ID`, `tipusok`.`tipusnev` AS `Kerékpártípus`, `tipusok`.`meret` AS `Méret` FROM `tipusok` ;

-- --------------------------------------------------------

--
-- Nézet szerkezete `tulajdonos_`
--
DROP TABLE IF EXISTS `tulajdonos_`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `tulajdonos_`  AS SELECT `tulajdonos`.`tulaj_id` AS `ID`, `tulajdonos`.`nev` AS `Név`, `tipusok`.`tipusnev` AS `Kerékpártípus`, `tulajdonos`.`telefon` AS `Telefonszám` FROM (`tulajdonos` join `tipusok` on(`tipusok`.`tipus_id` = `tulajdonos`.`tipuskod`)) ;
